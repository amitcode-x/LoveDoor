...
import { getSecondaryHero, updateSecondaryHero } from "../../api/adminApi";
import Card from "../../components/UI/Card";
import Button from "../../components/UI/Button";

export default function SecondaryHeroEdit() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const res = await getSecondaryHero();
        setForm(res.data);
        setExistingImage(res.data.final_image || "");
      } catch (err) {
        console.error(err);
        setError("Failed to load secondary hero data.");
      }
      setLoading(false);
    }
    load();
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      Object.keys(form).forEach((key) => {
        if (key === "button_link") return; // read-only
      ...
      const res = await updateSecondaryHero(fd);
      setSuccess("Secondary Hero updated successfully!");
      setForm(res.data);
      setExistingImage(res.data.final_image);
    } catch (err) {
      console.error(err);
      setError("Failed to update Secondary Hero.");
    }
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-xs text-slate-400">
        Manage secondary homepage banner.
      </p>
